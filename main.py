...
while True:
    try:
        ...
        df_btc['c'] = pd.to_numeric(df_btc['c'])
        ...
        for sym, data in MARKETS.items():
            ...
            if data['active']:
                ...
                if (data['side']=='LONG' and prix <= data['sl']) or (data['side']=='SHORT' and prix >= data['sl']):
                    ...
                    icon = "✅" if pnl >= 0 else "❌"
                    side_label = "📈" if data['side']=='LONG' else "📉"
                    output_trades.append(f"{icon} {sym} {side_label} | 📦Lot:{data['lot']} | 💰PNL:{pnl:.2f}$ | 🚨SL:{data['sl']:.2f}")
                ...
                if is_hot_zone(df, side) and ((btc_trend=='BULL' and side=='LONG') or (btc_trend=='BEAR' and side=='SHORT')):
                    ...
                    output_trades.append(f"🔍 {sym} | Recherche setup...")

        # ================= AFFICHAGE PROPRE (FIXE) =================
        # On efface l'écran pour garder les paliers fixes
